...
| `dp F9` | `rust/web/Cargo.toml` `[dependencies]` keys `tower-http`, `gloo-net`, `gloo-timers` | **GATED on D-19 - see `## Decision-blocked rows`.** Pin back to tower-http 0.6 / gloo-net 0.6 / gloo-timers 0.3 to match what leptos, reqwest, kube-client and backon already build, unless the newer APIs are demonstrably required | n |
...
|---|---|---|
| `dp F9` (web tower-http / gloo-net / gloo-timers pins) | **BLOCKED-ON-DECISION D-19** (`[workspace.dependencies]` migration, unblocks WP-64) | D-19 answered and WP-64 landed. This is a version-pin row: after WP-64 the version lives in the root `[workspace.dependencies]` table, so doing it first means writing the pin twice and re-resolving the conflict. The other WP-65 rows do **not** genuinely depend on D-19 - `lazy_static`->`LazyLock`, the stale template files, the CI job and the test-module rename are all independent of where dependency versions are declared - and stay in the main tables with sequencing notes. |
