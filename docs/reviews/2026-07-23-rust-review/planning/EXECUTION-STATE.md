...
## PROCESS (current)
NO COMMITS OR PUSHES IN-SESSION (user directive 2026-07-29, second session): each package's commit is recorded in ~/Development/brdgme-commit-plan.md for a later commit session. Changes stay uncommitted in the working tree. EXCEPTION 2026-07-29: user authorized commits+push for the wrap-up of this session's outstanding changes. DO NOT run rust-test.sh in-session on beefsack-laptop - the web target (web-ssr compile/link) OOMs the machine even at jobs=1; full gates there are run by the user. Targeted per-crate `cargo test -p <crate>` / `cargo clippy -p <crate>` runs are fine, and other machines have no restriction on rust-test.sh. If a gate log is ever needed, redirect to a persistent repo-root log (not tmp). DISK RULE: any disk-pressure error = STOP and report; never attempt cleanup. Safety ref: branch `pre-catchup-backup`.
