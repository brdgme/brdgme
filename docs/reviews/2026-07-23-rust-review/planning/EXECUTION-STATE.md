...
## PROCESS (current)
NO COMMITS OR PUSHES IN-SESSION (user directive 2026-07-29, second session): each package's commit is recorded in ~/Development/brdgme-commit-plan.md for a later commit session. Changes stay uncommitted in the working tree. EXCEPTION 2026-07-29: user authorized commits+push for the wrap-up of this session's outstanding changes. DO NOT run rust-test.sh in-session on this machine - repeated OOM kills the session even at jobs=1 (web-ssr compile); gates are run by the user. If a gate log is ever needed, redirect to a persistent repo-root log (not tmp). DISK RULE: any disk-pressure error = STOP and report; never attempt cleanup. Safety ref: branch `pre-catchup-backup`.
