...
| R-54 | partial/parked(U8 required GitHub check) | 8017a7f7, 9a2431ab | U9: private send-result metric branch increments `login_emails_sent_total` only after Resend success, increments `login_emails_failed_total` on failure, and has a DB-free Err-branch test. U14: `parse_reply_address` terminates reserved `i-noreply` before invite routing, with a direct parser test. `git diff --check` passed per unit; final `SQLX_OFFLINE=true cargo check -p web --all-targets --features ssr` passed with known warnings. Runtime tests are CI-pending. U8 remains parked for the later parked-item review: dated read-only GitHub API evidence 2026-08-04 shows no required checks exist - `GET /repos/brdgme/brdgme/branches/master/protection` -> 404 "Branch not protected", `GET /repos/brdgme/brdgme/rulesets?includes_parents=true` -> no rulesets, `GET /repos/brdgme/brdgme/rules/branches/master` -> `{rules: []}` - so e2e cannot gate merges/deploys today. No workflow change is made because removing `continue-on-error: true` alone cannot satisfy AC1 (merge-gate) without a required `e2e` check configured on the branch first; that configuration remains the pending external step. R-55 is next by tracker sequence; do not start it in this package. |
