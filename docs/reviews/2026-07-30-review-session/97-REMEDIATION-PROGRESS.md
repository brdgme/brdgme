...
| R-54 | partial/parked(U8 required GitHub check) | 8017a7f7, 9a2431ab | U9: private send-result metric branch increments `login_emails_sent_total` only after Resend success, increments `login_emails_failed_total` on failure, and has a DB-free Err-branch test. U14: `parse_reply_address` terminates reserved `i-noreply` before invite routing, with a direct parser test. `git diff --check` passed per unit; final `SQLX_OFFLINE=true cargo check -p web --all-targets --features ssr` passed with known warnings. Runtime tests are CI-pending. U8 is parked for the later parked-item review pending external confirmation/configuration that `e2e` is a required GitHub check; no workflow, job dependency, or deployment-documentation change. R-55 is next by tracker sequence; do not start it in this package. |
