...
- `pending` / `blocked(<reason>)` / `parked(<decision>)` / `in-progress` /
  `partial/parked(<reason>)` / `done(<commit>)` / `owner-gap` /
  `closed(<reason>)` - remediation item closed by ruling; any residual scope
  moves to `docs/BACKLOG.md` rather than continuing here /
  `deferred(<condition>)` - not parked indefinitely; runs once a stated
  condition is met
...
|---|--------|-----------|-------|
| R-01 | done(cc4582e5) | cc4582e50c82075864f322df4199787b5793b9e3 | ships with R-02; compile-verified post-hoc (check/clippy/fmt all pass); commit tag corrected from orphaned rebase duplicate a8029b82 (not an ancestor of HEAD) to identical-patch ancestor cc4582e5 |
...
| R-21 | done(3f26f1d7) | 3f26f1d7b895889db0204bab925b02691ef2810b | Closes F-06 at the trait boundary: `Gamer::validate` is required with no fail-open default. All 28 game crates compile with an explicit implementation: 27 supported games retain their existing validation, while `lords-of-vegas-1` has the owner-approved localized WIP `Ok(())` exception explicitly commented as not claiming complete validation. Test-only `Gamer` mocks in `brdgme_cmd` and web are likewise explicit. The R-22..R-26/R-48 and 5.5 predecessor evidence supplies the direct malformed-state validation coverage; no validation or gameplay rules were invented here. Lead gates passed: `cargo test`/clippy/fmt for `brdgme_game`, `lords-of-vegas-1`, and `brdgme_cmd`; all 28 game packages serially `cargo check -p`; allowed `SQLX_OFFLINE=true cargo check -p web --tests --features ssr`; `git diff --check`. Independent review PASS with no Critical/Important findings; the web check has only known unrelated warnings. |
| R-22 | done(1027e9d8) | 1027e9d828723e31e59acacf8a8289c846b2d31f | Owner-amended AC1 covers exactly four real per-player arrays: `player_hands`, `player_money`, `bets`, `folded_players`. `validate()` rejects each short-by-one array, invalid player/index/card-count/negative-money/negative-bet states; the `status()` no-panic test uses short `player_money`, which previously panicked in the raw-indexed path and is now invalid-state safe. F-44 gates `can_raise` on `min_raise()` so the parser cannot advertise inverted bounds; F-45 sorts winner keys before showdown logs. The direct folded-hole-card 5.5 log test is retained. `cargo test -p texas-holdem-2` passed 50 unit + 1 contract; targeted clippy/fmt and `git diff --check` passed. Independent review accepted all R-22 ACs; its one AC2 test-degeneracy finding was repaired and re-reviewed. No gameplay/rules change. |
...
| R-31 | parked-by-user | | Owner ruling: retain the 8-player cap because the platform supports only eight pre-approved player colors; 9-10 players are unsuitable now. Do not implement the published 2-10-player variant. The broader player-limit issue may be revisited in a future per-game review. F-73 remains parked because it cannot proceed under this indivisible R-31 plan; no source/doc implementation occurs. Confirmed final by user ruling 2026-08-06 (not reopened): 8 players is a hard platform limit for now; expanding the player colour palette is future work outside this remediation effort. |
| R-32 | done(0091658) | 932b7e79ad8dd610698e84c97c6a2b9eef4314e2, eb811900d418428b9c200f35590bbcec05dd8d84, 4f97c92f798f9f6ab76abc0ba92feaada9074d87, 2d245b696dcdf3cff4c073bab24749d38d87c12f, 0091658b299ea399b595cf1da3702ae2a2eb6356 | Closes F-18/F-71/F-52/F-19/F-21/F-56. F-20 is N/A: Starship Catan is fixed two-player, so `0..2` placings remains correct; no source, gameplay, player-limit, or non-two-player-fixture change. AC1 static sweep found zero ungated epilogues; AC2 direct finish-path tests in all 19 affected crates assert one final epilogue (and Sushizock one end log); AC3 deduplicates placings in all five affected crates; AC5 Acquire regression and AC6 Cathedral finished command-spec tests pass. Serial per-crate test/clippy/fmt gates and `git diff --check` pass. Independent review found no blocker; parser rejection makes a second finished command unavailable, limiting direct true-branch guard coverage without a new unapproved harness. |
...
| R-37 | closed(rest moved to backlog) | 0270f296a39755b44feacf85d6d2220d7c8b4f80 | R-37.0 complete. User ruling 2026-08-06: R-37 CLOSES as a remediation item. R-37.1 and R-37.2 (auth/session integrity; rate-limit and external-auth hardening, closing F-94 - no rate-limiting middleware anywhere in `rust/web`) MOVE to `docs/BACKLOG.md` #60 as pre-go-live work, carrying their already-approved specs recorded in `98-REMEDIATION-PLAN.md`'s R-37.1/R-37.2 sections and this row's R-37.0 evidence - do not re-derive. Preserve R-37.1 before R-37.2 when implemented. |
| R-38 | done(fac14fdf) | fac14fdf9f6b546d5433095bc0c1fc5b48ca3779 | Closes F-98/F-99/F-106/F-103. F-97 retains arbitrary admin-configured HTTP/HTTPS providers as an explicit trusted-admin accepted SSRF/infrastructure-access risk, with no URL/network/crypto change; revisit if config becomes untrusted, admin roles are delegated, or sensitive internal HTTP services are exposed. F-100 retains cookie-only expiry; the 40-day remediation-only assertion/comment was removed while issued/revoked/nonexistent session checks remain. F-99 audit confirms behavioral coverage for all 26 reminder-named DB functions; reminder retained as reachability/population guard. F-98 validates blank/oversized keys before create/update encryption with focused tests. F-106 renders/logs a bounded reqwest error-source chain with an in-process streamed-body regression test. F-103 returns contextual missing-`DATABASE_URL` Err. `git diff --check` and final `SQLX_OFFLINE=true cargo check -p web --all-targets --features ssr` pass (known unrelated warnings); independent review PASS after F-106 correction. Runtime web/DB tests CI-pending. |
...
| R-43 | closed(stay on 0.7.0; CI job backlogged) | 785cd855a7fb82930d6ecee9c4224cfb7ab28052 | User ruling 2026-08-06: stay on tower-http 0.7.0 per the dependency-currency stay-on-latest policy (`docs/CODING.md:540`); AC2's downgrade-to-0.6.11 criterion does not apply and R-43 needs no dependency change. The `deny.toml` skip-entry justification is corrected (785cd855: explicit first-party-pin/stay-on-latest annotation, not a papered-over sibling). Adding `cargo deny check bans` (and licenses) to the weekly CI job (AC1) is BACKLOGGED as `docs/BACKLOG.md` #62, not done. WP-69 §5's parked negative checks (AC4) remain recorded/unresolved per D3; see R-43 parking below for the full disposition. |
| R-44 | done(3fc8d79e) | 3fc8d79e1198904d5c88f91f3a4d786cce3c048f | Closes F-200/F-201/F-202. **F-200:** `migrate()` now classifies the concurrent-`create schema` loser's error via `sqlx::Error::Database(db_err) if db_err.is_unique_violation()` (SQLSTATE 23505) instead of a `Display` substring match, then rolls back the aborted transaction and creates the table in a fresh transaction instead of returning `Ok(())` early. First fix attempt (fall through to `create table` in the same tx) failed empirically: Postgres aborts a transaction on ANY error, so the loser's `create table` returned 25P02 - reproduced via two held psql sessions (`duplicate key value violates unique constraint "pg_namespace_nspname_index"` then `current transaction is aborted`); the rollback+re-begin fixes it. `Ok(())` is now only reachable after a committed `create table`; a pre-existing schema-without-table half-migrated state also self-heals on the next `migrate()`. **AC1** `concurrent_cold_start_migrate_creates_session_table`: two connections cold-start `migrate()` on a fresh `#[sqlx::test]` DB; table exists afterwards. Independent review note: AC1 is a concurrency smoke test and not discriminating against buggy code - it can pass against the defect when the winning side happens to create the table. **AC2** `migrate_does_not_report_success_when_session_table_missing`: control connection holds an uncommitted schema row forcing the duplicate-key branch; `migrate()` now returns Ok only with the table present (asserts `Ok` implies `session_table_exists`); this is the discriminating red regression. Against the defect, was 1 passed / 1 failed RED (AC1 pass, AC2 fail); now green, stable 5/5 isolated runs (0.4-0.5s each). `wait_for_blocked_create_schema` poll is scoped to the current test database (`datname = current_database()`). **F-201 recheck (all three workspace sites + the migrate second-order site; direct sqlx 0.8.6 vs 0.9.0 source comparison):** (1) `web/src/db/users.rs:256` `e.code().as_deref()==Some("23505")` -> still fires; (2) `web/src/db/emails.rs:103` same pattern -> still fires; (3) `web/src/game/import.rs:240` `de.is_unique_violation()` -> still fires; (4) `session_store` migrate substring match -> REPLACED by `is_unique_violation()`. Evidence: `PgDatabaseError::code()` returns `Some(Cow::Borrowed(self.code()))` (raw SQLSTATE) identically at sqlx-postgres 0.8.6/error.rs:170 and 0.9.0/error.rs:174; `kind()` maps `23505`->`UniqueViolation` identically (0.8.6:211, 0.9.0:215); `DatabaseError::is_unique_violation()` default `matches!(kind(), UniqueViolation)` identically (sqlx-core 0.8.6/error.rs:252, 0.9.0/error.rs:260); `sqlx::Error::Database` variant and `dyn DatabaseError` structure unchanged. The ONLY 0.9 change in this area is a new `ErrorKind::ExclusionViolation`/23P01 arm - irrelevant to all four classifications. No change made at users.rs/emails.rs/import.rs. **F-202:** `count_rows` (`web/src/db/test_support.rs:146-152`) no longer interpolates `table: &str` behind `AssertSqlSafe`; it now matches each table name to a compile-time `&'static str` SQL constant (`friends`, `games`, `game_players`, `game_bots`, `game_logs`, `game_log_targets`, `users` - every caller site audited) and panics on an unlisted name; `&'static str` implements `SqlSafeStr` natively (sqlx-core 0.9.0/sql_str.rs) so no wrapper remains. **Gates:** `cargo test -p brdgme_session_store` 2 passed/0 failed (+2 ignored doctests); `cargo fmt -p brdgme_session_store -- --check` exit 0; `cargo clippy -p brdgme_session_store --all-targets -- -D warnings` exit 0; `git diff --check` exit 0. `test_support.rs` edit sanity-checked via single-file `rustfmt --edition 2024 --check` (exit 0) only - no web cargo operation run per brief. Uncommitted per work-unit brief; independent review recommended (auth-adjacent, new concurrency/rollback semantics). |
| R-45 | done(af71e05) | af71e05fd31b86691c27d48bf9c66aa94fbb8d6e, a2e1b9402e792f16183bd8e6d4547afdcbcc9ffa | AC1 complete: `blocked_domains.rs` is a vendored verbatim copy of `disposable-email-domains/disposable-email-domains` `disposable_email_blocklist.conf` at immutable `f9004b00673ca09626e9dd5eaf97fd657d009cb8` (2026-07-22); local list byte-identical (`diff -q` clean; sha256 `79f6c4911ee397d92d40584ecce12e0de7865714024edf53e819f708232c0f8e`); CC0 1.0 upstream text recorded verbatim in sibling `rust/web/src/auth/BLOCKED_DOMAINS_LICENSE` (sha256 `71e4e05374bd16288e638ede168482f8642204fdb90aede848c9fcdf44730026`). R45.2's focused CI guard now also checks the blocked-domain licence phrase and exact immutable commit (see R-45 evidence AC3). No upstream-defect assessment claimed for `blocked_domains.rs` (AC4 scope unchanged). |
| R-46 | closed(R-46.0 only; rest to backlog) | 0b7bce71e3fa4d7a5a6a483badb6df30ca6649c1, a425af2048157e2203c9f8883d3475cb5cc050d1 | R-46.0 corrected F-197's stale count to two remaining `.rls.toml` files, citing WP-65 (`2c28ae8`) for the three prior deletions, then removed both. `rg --files -g '.rls.toml' rust` returned no paths; `git diff --check` passed. User ruling 2026-08-06: R-46 CLOSES at R-46.0. R-46.1..R-46.4 (the non-empty rust-lint baseline, threshold/policy choices, and owner decision 6.3a's eliminate-all-22-`allow`-overrides expansion) MOVE to `docs/BACKLOG.md` #61 as a standalone quality item, not implemented here. R-47 is the next unblocked package. |
...
| R-50 | done(7652e6ca) | 7652e6ca2e355966598e521348aed081bc6b95df | Closes F-32/F-53/F-75/F-77/F-78/F-79/F-80/F-83/F-84 (accepted, uncommitted). **F-79** (farkle-2/src/render.rs): `combination_name` unlisted combination is now `unreachable!`, so a new `scores()` entry fails loudly instead of silently rendering "Unknown"; `test_scoring_table_matches_scores` derives the table from `scores()` and asserts exact row parity plus `len == scores().len()+1` (replaces the hardcoded legacy list); `#[should_panic]` `test_combination_name_rejects_unknown_combination`. RED evidence: pre-fix `display_name(&[2, 3])` returned "Unknown" without panicking, so the should-panic test failed. **F-83** (roll-through-the-ages-2/src/lib.rs): `keep_skulls` now returns `(Vec<Log>, bool)` phase-advance signal; `roll_n`/`reroll` use `!advanced` instead of the phase-equality guard, so an all-skulls cascade that wraps back to Roll no longer decrements the next player's `remaining_rolls`; full-cascade test `test_roll_all_skulls_advances_player_without_decrementing_next` asserts STEVE is at Phase::Roll with 2 rolls. RED evidence: pre-fix the phase-equality guard left remaining_rolls at 1, test failed. **F-78** three raw-add sites to zero via saturated `banked`: farkle-2/src/lib.rs `turn_score.saturating_add(value)` on take; `banked = scores[player].saturating_add(turn_score)` feeds both the log display and `scores[player] = banked` on done; greed-2/src/lib.rs same `banked` on done. **F-84** (seven-wonders-1/src/lib.rs): `boards.len() < players` -> `GameError::internal` guard before the `boards[..players]` slice; no-duplicate-board test loops `MIN_PLAYERS..=MAX_PLAYERS` (3..=7). **F-53** (sushizock-2/RULES.md): setup reconciled toward the published rules - both tile rows now "laid out face up in a row" (was face down), agreeing with DATA_DOCS.md's central-row wording; no code change. **F-80** (liars-dice-2/src/command.rs): bid quantity parser `max: None` (over-cap bids are legal bluffs) with updated desc; `bid_parser_accepts_every_boundary_bid` enumerates `[MIN_BID_QUANTITY, cap, cap+1, i32::MAX]`. **F-75** (zombie-dice-2/src/lib.rs): duplicated inline turn reset extracted to `reset_turn`, used by both `start_turn` and the rolloff loop. **F-32** (starship-catan-1/src/lib.rs + render.rs): `PlayerBoard::pub_view()` explicit per-field public projection replaces `player_boards.clone()` in `pub_state`; blocklist `collect_keys` test replaced by an allow-list BTreeSet asserting the exact PubState key set. **F-77** (red7-1/src/lib.rs): stale `end_points` doc comment corrected (out-of-range `num_players` now rejected by `Gamer::validate`; saturating ops documented as defence in depth). Final acceptance evidence: focused `cargo test -p` pass for all nine affected crates - starship-catan-1 48, sushizock-2 54, zombie-dice-2 28, greed-2 17, farkle-2 19, liars-dice-2 10, roll-through-the-ages-2 119 (118 unit + 1 contract), seven-wonders-1 52, red7-1 28; `cargo clippy -p <crate> --all-targets -- -D warnings` clean all nine; per-package `cargo fmt -p <crate> -- --check` clean; `git diff --check` clean. No independent review claimed. |
| R-51 | done(4ed1aa7c) | 4ed1aa7cf13cd8525d25d14462e66240002ae479 | F-196 accepted: immutable 031 adds nullable per-version `player_counts`/`weight`/`blurb` snapshots; operator backfills incomplete snapshots despite an unchanged observed generation, then resumes its guard; registration atomically re-points descriptors after apply/deprecation/cleanup/demotion from the newest fully snapshotted public, non-deprecated version (`created_at DESC, name DESC`). Availability remains public/non-deprecated-only during backfill. Focused registration/operator checks and allowed web check pass; DB runtime tests are CI-only. F-140 remains parked and untouched. |
| F-140 | done(8094a997) | 8094a99770e927a908789f64369b7b28903b3153 | Public deprecated versions now remain rules-readable through the two shared `game_types` lookup predicates, while non-public versions return neither rules nor render metadata. DB regressions call both production lookups for public-deprecated success and non-public failure in both deprecation states; an anonymous in-process SSR regression drives `/rules/<version_id>` with authored public-deprecated V1 rules. Static audit confirms page/email callers remain shared and new-game selectors remain public/non-deprecated. `git diff --check` and `SQLX_OFFLINE=true cargo check -p web --all-targets --features ssr` pass with only known unrelated warnings; runtime DB/SSR tests are CI-only. Independent access-control/test-quality review found no actionable F-140 issue; its two scope reports were the pre-existing R-51 F-196 hunks, not F-140. No staging, commit, or push. |
...
| R-53 | done(1b73f665) | 1b73f6657d82bbf1aac1a621628ab763e3069d57 | `points()` is documented as public human-facing score/state data with game-specific direction; `Status::Finished.placings` is the sole result/rating authority, including ties and game-specific tie-breakers. Category 5, No Thanks, and Cathedral retain positive lower-is-better raw points; focused raw-score, placing, finished-status, and tie tests cover Category 5/No Thanks, and Cathedral covers raw scores, placing/tie metrics, and its positive shared final-score log. Inventory: 28 Rust `Gamer` implementations, 26 `points()` overrides, 23 higher-is-better raw values, three lower-is-better games, and empty defaults only in Seven Wonders and WIP Lords of Vegas. The web manual-end `points DESC` fallback was a separate owner decision outside R-53, and R-53 itself made no web change; the fallback was later removed by `44c057b4`, so a last-human stop now has no authoritative game placing - every seat's `place` is cleared, `end_reason = 'last_human_stop'` is recorded, and competitive ranks come solely from `write_ranked_placings`. Targeted `brdgme_game`, `cathedral-2`, `category-5-2`, and `no-thanks-2` fmt/clippy/tests pass. Independent shared-contract/game-result review found no source defect. |
| R-54 | closed(U8 moved to backlog) | 8017a7f7, 9a2431ab | U9: private send-result metric branch increments `login_emails_sent_total` only after Resend success, increments `login_emails_failed_total` on failure, and has a DB-free Err-branch test. U14: `parse_reply_address` terminates reserved `i-noreply` before invite routing, with a direct parser test. `git diff --check` passed per unit; final `SQLX_OFFLINE=true cargo check -p web --all-targets --features ssr` passed with known warnings. Runtime tests are CI-pending. User ruling 2026-08-06: R-54 CLOSES. U8 (turning off `continue-on-error: true` so e2e gates merges/deploys) MOVES to `docs/BACKLOG.md` #63, BACKLOGGED behind fixing the hydration-race flake first - dated read-only GitHub API evidence 2026-08-04 already showed no required checks exist (`GET /repos/brdgme/brdgme/branches/master/protection` -> 404; `GET /repos/brdgme/brdgme/rulesets?includes_parents=true` -> no rulesets; `GET /repos/brdgme/brdgme/rules/branches/master` -> `{rules: []}`), so e2e still cannot gate merges/deploys today. R-55 is next by tracker sequence. |
| R-55 | done(b2fbdc4e) | b2fbdc4e | `r55-u10` covered-clean for `rust/bot/src/routing.rs`; possible Vision round-robin mismatch is a separate confirmation candidate, not a finding. `r55-u15a` complete: line-level audit of all 37 `proposals.rs` tests found 34 direct calls and three tooth-3 misses for later routing only: `transfer_rejects_bot_and_nonplayer_targets` calls only valid `update_proposal_owner`, `start_guards_reject_pending_declined_invalid_count` re-implements pending/declined predicates rather than calling `start_proposal`, and `declined_to_accepted_is_rejected` calls no production function. `r55-u15b` complete: line-level audit of all 33 `email/sweep.rs` tests found 32 direct production-target calls/references, no indirect-only tests, and one tooth-3 miss for later routing only: `reset_reminder_clears_timestamp` has no production-function call, instead clearing and reading `turn_reminder_sent_at` with raw SQL. `r55-u16a` complete: Lead line-reviewed `admin.rs:1560-3488`; 30 test functions begin in-range, with 28 direct production calls and two high-confidence Low tooth-3 candidates for later routing: `test_admin_list_bots_rejects_non_admin` (`:2503-2521`) calls only `is_user_admin`, not `admin_list_bots`; `test_admin_list_dangling_bot_names_rejects_non_admin` (`:2524-2542`) likewise calls only `is_user_admin`, not `admin_list_dangling_bot_names`. This invalidates 05b's rejection-coverage claim (`05b-web-admin-bot-db.md:617-622`); the real `admin_list_bots` three-auth-state test is `:2475-2500`, while the latter server fn has no direct-call rejection test. All in-range components and the other in-range tests were covered-clean; no source change. AC1 is complete for u16a. `r55-u16b` complete: `rust/web/src/db/game_write.rs` (:1-2794) read at line level, including `update_game_command_success` (:698-810). Previously-read regions were the known F-111..F-120, F-127, F-148 sites; the current source already carries their WP-40 remediation (all four lifecycle writers open with `claim_unfinished_game_tx`; `pick_replacement_bot` is tx-scoped; `undo_game` restores points and clears `left_at` on un-elimination; `concede_game_replace` preserves `is_turn` and fires the `updated_at` trigger via a no-op UPDATE), so none were re-filed. `update_game_command_success` matches the reference claim shape (CODING.md:653-667): `AND updated_at = $5` -> `StaleStateConflict` on 0 rows, sticky `is_finished`/`finished_at` COALESCE (ws F37), guarded `is_eliminated`/`left_at` CASEs (ws F6), deliberate undo-stash and `is_turn_at`/`turn_reminder_sent_at` semantics (ws F44); its only production caller `execute_command` gates `is_finished` (`game/mod.rs:104`), so finished-game writes are unreachable except the race the sticky guard covers. No request-reachable panics in the production section; `delete_game` FK ordering and the documented `game_proposals` manual `updated_at` (ws F36) verified. Tests: 31 in module, 30 with direct production calls; `create_game_with_users_assigns_positions_and_colors` exercises `create_game_with_users` via its sole fixture wrapper `make_game_with_players` (`test_support.rs:48-65`) - a real behavioral test, not a tooth-3 miss. Result: covered-clean, no new finding; no Cargo/test command run; `git diff --check` passed. `r55-u17` complete: owner ruling settles the previously blocked AAD disposition - verbatim: "WP-36 recovered spec is operative. Retain no-AAD as an accepted risk; revisit before a new encrypted data category, credential movement/import, or multiple interchangeable ciphertext contexts. No crypto migration now." (spec: `868094a6:docs/reviews/2026-07-23-rust-review/planning/specs/archive/WP-36-crypto-deploy-hardening.md`). R-55 done; uncommitted per work-unit brief. This resolution supersedes the `r55-u17` "partially blocked" note in 98-REMEDIATION-PLAN.md's dated 2026-08-03 execution-units snapshot, which is left unedited as an immutable record. |
| rrm-r55-directness-tests | no-action(all 6) | | Value/cost review of R-55's six recorded tooth-3 directness candidates; approved package covered add-test-or-no-action only, no source change. All six are no-action, each already closed by other existing direct-call tests or lacking a testable production seam: (1) `transfer_rejects_bot_and_nonplayer_targets` (`proposals.rs:3357`) - redundant; `transfer_target_must_be_accepted_human` (`proposals.rs:3116-3149`) already calls `transfer_target_error` directly for accepted/pending/declined/not-found, and a bot target and a random non-player target both hit the same not-found branch as the existing `Uuid::new_v4()` case. (2) `start_guards_reject_pending_declined_invalid_count` (`proposals.rs:3641`) - its invalid-count assertion already calls the real `roster_error` directly (`:3701`); the pending/declined branches are inlined in `start_proposal` with no extractable pure guard, so direct coverage would require the NATS-dependent `with_start_proposal_context` harness, which cannot be verified in this environment (no local NATS; `cargo test -p web` is session-banned per this file's Machine constraints, line 14) for guard logic whose triggering conditions are already exercised against real seeded state - disproportionate, unverifiable cost for a trivial branch. (3) `declined_to_accepted_is_rejected` (`proposals.rs:3772`) - redundant; `respond_denied_reason_blocks_owner_and_bad_transitions` (`proposals.rs:3097-3113`) already directly calls `respond_denied_reason(false, "declined", "accepted")` and asserts the exact rejection message. (4) `reset_reminder_clears_timestamp` (`email/sweep.rs:965`) - no dedicated "reset reminder" production function exists; `turn_reminder_sent_at` is only cleared as a side effect inside the larger lifecycle writers in `db/game_write.rs` (already covered by their own dedicated tests per R-32/R-38/R-55 u16b); the raw-SQL-only test exercises no application logic. (5) `test_admin_list_bots_rejects_non_admin` (`admin.rs:3061`) - redundant with the pre-existing `admin_list_bots_distinguishes_anonymous_non_admin_admin` (`admin.rs:2509-2535`), which the R-55 row above already cites as the real coverage. (6) `test_admin_list_dangling_bot_names_rejects_non_admin` (`admin.rs:3082`) - the R-55 row's claim that this fn had no direct-call rejection test is now stale: `admin_list_dangling_bot_names_distinguishes_anonymous_non_admin_admin` (`admin.rs:2537-2568`) already covers it, added by the later "5.3 require_admin true-path tests" package (line 141 above), which explicitly records closing this exact R-55 u16a gap. Verification: direct source inspection only (`rg`/`sed`/`Read` of the cited line ranges); no cargo command run, no test/source file changed; `git status`/`git diff` unaffected. No commit boundary proposed. |

## R-43 parking

- **Status:** resolved by user ruling 2026-08-06 (see the R-43 row above). No longer parked; do not treat as pending.
- **Governance conflict (resolved):** active latest-dependency governance conflicted with R-43 AC2's `tower-http` downgrade. Resolved: stay on tower-http 0.7.0 per the dependency-currency stay-on-latest policy (`docs/CODING.md:540`); AC2's downgrade criterion does not apply.
- **Unresolved evidence (addressed):** all 29 `deny.toml` skips and the falsified all-upstream rebuttal were reviewed; the `tower-http` skip-entry comment is corrected (785cd855). The remaining 28 skips are unchanged; a review-date cadence for them remains unselected.
- **Disposition:** running `cargo deny check bans` in CI (AC1) is BACKLOGGED as `docs/BACKLOG.md` #62, not implemented. WP-69 §5's parked negative checks (AC4) remain recorded/unresolved per D3. No further R-43 acceptance criterion is claimed complete.
- **Correction note (added 2026-08-07):** user ruling 2026-08-07 splits R-43's AC3 (from `98-REMEDIATION-PLAN.md`) in two. The stale-skip-detection half is now implemented: `-D unmatched-skip -D unnecessary-skip` is enforced in `.github/workflows/ci.yml`'s `cargo-deny` job, proven by the negative test in this change (a bogus skip entry matching no crate in the graph fails the check with `error[unmatched-skip]`; a skip for a single-version crate fails with `error[unnecessary-skip]`). The expiry-date half is CLOSED AS SUPERSEDED: cargo-deny 0.19.9's `[[bans.skip]]` schema has no expiry/TTL/date field, `unnecessary-skip` already detects the real failure mode (a skip no longer needed), and a bespoke expiry-date convention plus custom tooling would only add calendar-driven churn for a 29-entry file with no additional detection value. No backlog entry - it is closed, not deferred.

## R-45 evidence

- **Status:** done (accepted, uncommitted). The `blocked_domains.rs`
  provenance gap that parked R-45 is closed: the list is pinned to an
  immutable upstream commit with verbatim CC0 licence text locally recorded.
  This package does not select the next package.
...
| `rust/web/src/auth/blocked_domains.rs` | `disposable-email-domains/disposable-email-domains` `disposable_email_blocklist.conf` at immutable commit `f9004b00673ca09626e9dd5eaf97fd657d009cb8` (2026-07-22, `Add missing domains from sources (#1102)`); attribution header at `:1-6`; introduced by `ed4bedb`; local list byte/line identical (`diff -q` no output; both sha256 `79f6c4911ee397d92d40584ecce12e0de7865714024edf53e819f708232c0f8e`) | CC0 1.0 Universal; upstream text recorded verbatim in sibling `rust/web/src/auth/BLOCKED_DOMAINS_LICENSE` (sha256 `71e4e05374bd16288e638ede168482f8642204fdb90aede848c9fcdf44730026`) | Static disposable-domain blocklist | No | Not assessed: R-45 AC4 applies only to the confirmed vendored-code directory |
...
- **AC1:** complete. The inventory is complete for the approved boundary, and
  the copied blocklist now records its copy-time version (`f9004b00673ca09626e9dd5eaf97fd657d009cb8`,
  source path `disposable_email_blocklist.conf`) and licence (CC0 1.0, text in
  sibling `rust/web/src/auth/BLOCKED_DOMAINS_LICENSE`).
- **AC2:** complete. Every inventory entry states whether its obligation is
  machine-checked.
- **AC3:** complete. The `session_store` single-assertion guard landed at
  `a2e1b9402e792f16183bd8e6d4547afdcbcc9ffa`; it now also checks the
  `blocked_domains.rs` licence and attribution. `.github/workflows/ci.yml:101-103`
  runs the approved single guard (`Check vendored licence and attribution`)
  asserting the `session_store` MIT licence text and exact upstream attribution
  header plus the blocked-domain CC0 licence phrase (`Public Domain Dedication`
  in `rust/web/src/auth/BLOCKED_DOMAINS_LICENSE`) and exact immutable upstream
  commit (`f9004b00673ca09626e9dd5eaf97fd657d009cb8` in
  `rust/web/src/auth/blocked_domains.rs`). The guard and `git diff --check`
  passed locally; no Cargo command was run.
...
|------|--------|-----------|-------|
| 5.1 roll-through-the-ages-2 crate pass | done(d06c60b6) | d06c60b627c6470b437d4636aa4aba21145715a4 | Audit (no source change) per Units 02-04 terms: PubState{game} lib.rs:105-111 / PlayerState{game,player} :113-120, all 13 Game fields pub :59-78; all 25 log sites are `Log::public`, none player-specific; `boards` is the sole per-player vec, length always == players (start :1514-1519), no divergent parallel vectors, `log_roll` chain split is index-safe :694-703; indexes bounded (current_player kept in range by next_turn modulo :571; command/`player` pre-validated requester/gamer.rs:24-33; roll dice bounds 0..=l :722-729). Coverage gaps (notes, not findings): no `validate` override (default is no-op game.rs:106-108); no redaction/no-panic tests (parity absent vs alhambra-1:2094, sushizock-2:1954); only test is `assert_gamer_contract` (tests/contract.rs). **Concrete finding - unplanned follow-up, NOT R-50:** `Game.rng` (lib.rs:77, serde default `from_entropy` :76) serializes through the full-clone `PubState`/`PlayerState` (`pub_state`/`player_state` :1524-1533) and is a deterministic ChaCha8 stream whose position serializes (rng.rs:23-24, :79-86; rng.rs:3-12), so a holder of that serialized state can predict future dice (`roll_n`, lib.rs:642-649). Not a live exploit: the current architecture does not deliver those states to ordinary clients - the web layer passes the game-service `render` HTML and `command_spec` to clients, never the serialized `pub_state`/`player_state` (game/server_fns.rs:271-279, :352-400); the game service is internal ClusterIP behind `*.games.internal` (k8s/base/game/roll-through-the-ages-2/service.yaml; http-scaled-object.yaml:7), and the serialized state is internal game-service/DB (`games.game_state`, migrations/001_initial_schema.sql:178)/admin-export state. Classified low latent defense-in-depth/convention-divergence risk, not a live exploit. Programme convention redacts rng: red7-1 test asserts `json.get("rng").is_none()` (red7-1/src/lib.rs:862-865); age-of-war-2 builds explicit PubState without rng (age-of-war-2/src/lib.rs:478-490). Fix would need rng redaction in both states plus a redaction test (crate has no serde_json dev-dep). Do not implement here. Dependency: R-50 (F-83) covers the previously-read finish/placings function only; untouched. Verification: `cargo test -p roll-through-the-ages-2` green (118 unit + 1 contract), `git diff --check` passed. |
| 5.2 session_store test module | done(3fc8d79e) | 3fc8d79e1198904d5c88f91f3a4d786cce3c048f | New `#[sqlx::test(migrations = false)]` harness (`rust/lib/session_store/tests/migrate.rs`) + dev-deps (sqlx `macros`+`migrate`, tokio; Cargo.lock +1 line); no production source change in the harness. AC1 `concurrent_cold_start_migrate_creates_session_table` and AC2 `migrate_does_not_report_success_when_session_table_missing` now PASS on the R-44 F-200 fix. Independent review note: AC1 is a concurrency smoke test, not discriminating against buggy code; AC2 is the discriminating red regression (was the 1 failed RED against the defect). `wait_for_blocked_create_schema` poll is database-scoped (`datname = current_database()`). Gates: `cargo test -p brdgme_session_store` 2 passed / 0 failed (stable 5/5), `cargo fmt -p brdgme_session_store -- --check` exit 0, targeted `cargo clippy -p brdgme_session_store --all-targets -- -D warnings` exit 0, `git diff --check` exit 0. Completed by R-44. |
| 5.3 require_admin true-path tests | done(b070a574) | b070a5744b7d112484004449201fbd7bfcf62b52 | 15 new three-auth-state `#[sqlx::test]` cases in `rust/web/src/admin.rs` `mod tests` (+523, test-only) close the "true path untested for 13 of 16 server fns" gap (plan 98-REMEDIATION-PLAN.md:2263-2272). Each drives the server fn itself (tooth 3 - never `require_admin` in isolation) through the 5.4 request-parts harness `crate::test_support::{anonymous, non_admin, admin}`, asserting anonymous -> `Not authenticated`, non-admin -> `ServerError(ADMIN_REQUIRED)`, admin -> success (mutating fns verify the row via SQL). New coverage: `admin_list_dangling_bot_names`, `admin_create_bot`, `admin_update_bot`, `admin_reorder_bots`, `admin_delete_bot`, `admin_list_providers`, `admin_create_provider`, `admin_update_provider`, `admin_delete_provider`, `admin_list_bot_providers`, `admin_create_bot_provider`, `admin_update_bot_provider`, `admin_delete_bot_provider`, `admin_test_provider`, `admin_test_bot_provider`; with the pre-existing `admin_list_bots_distinguishes_anonymous_non_admin_admin` (`:2475`), all 16 admin server fns now have a distinguishing true-path test. Also closes the R-55 u16a tooth-3 gap: `admin_list_dangling_bot_names` previously had no direct-call rejection test (only `is_user_admin`). Runtime `#[sqlx::test]` web cases compile-verified only; runtime deferred to CI (web build/test/run banned). Uncommitted per work-unit brief. |
...
| 5.5 current game-crate validation, redaction, and log matrix | done(bd4dd86c) | bd4dd86c677eb1f1751df04dc50ad33bc3c90373 | Accepted checked rows: acquire-1 (validate, malformed state, public play log); alhambra-1 (all present); cathedral-2 (public play log); jaipur-2 (public sale log and private bonus absence); lost-cities-1 (public/player serialized hand redaction and start-hand logs); roll-through-the-ages-2 (validate/malformed state, persisted RNG versus RNG-free render-state serialization, all-public logs); seven-wonders-1 (public build log and remaining-hand absence); splendor-2 (all present); starship-catan-1 (public buy log); sushi-go-2 (public hand/reveal log); sushizock-2 (documented-open-information public-roll/take logs); texas-holdem-2 (folded-hole-card public-log absence; validation belongs to R-22). `lords-of-vegas-1` is WIP owner-excluded, not passed and untouched. Serial target test/clippy/fmt gates passed for all changed crates, with `git diff --check` clean. Independent review found two Roll validation false positives (valid transient food/goods above later caps) and a missing seven-wonders positive log proof; both were repaired, rerun (Roll 126 unit + 1 contract; Seven Wonders 53 unit + 1 contract), and re-review passed. |
| 5.8 bot test coverage (U11, U12) | done(eda7407d) | eda7407dc44fdb83ff6aa07489a53ecf47bf0282 | `build_messages_renders_game_data_not_raw_game_state` calls the production two-player prompt builder and uses distinct public, acting-seat private, opponent-private, and full-Status/raw-game sentinels. The rendered user prompt contains public and acting-seat state but excludes opponent and full-Status state; no whole-prompt snapshot, production, protocol, points, or web change. `cargo test -p bot build_messages_renders_game_data_not_raw_game_state`, bot fmt/clippy, and `git diff --check` pass. Independent privacy/test-quality review PASS; the game-client R-35 mock remains the complementary endpoint-level proof that `fetch_game_data` requests no Status. |

## Deployment items (section 3, brdgme-config)

| Item | Status | Notes |
|------|--------|-------|
| F-96 Turnstile secret key (prod) | secretRef added(525f81a9); sealing outstanding | Option A: new `turnstile-config` Secret. `k8s/base/web/deployment.yaml` `secretRef` wired (525f81a9); carries `TURNSTILE_SITE_KEY`/`TURNSTILE_SECRET_KEY`. **Outstanding external action:** the operator must still seal the Secret itself in `brdgme-config` - not done by this session. |
| TURNSTILE_SITE_KEY startup check | done(525f81a9) | Fail-closed startup check added, mirroring the existing `TURNSTILE_SECRET_KEY` check, gated by the same `ALLOW_INSECURE_DEFAULT_KEY` escape hatch (`rust/web/src/main.rs`). |
| config::public_base_url() prod HTTPS | done(525f81a9), defence-in-depth | Fail-closed startup check added: refuses to start unless `public_base_url()` resolves to `https://`. Original premise was stale: `k8s/base/web/deployment.yaml:45` already sets `PUBLIC_BASE_URL=https://brdg.me` in prod; the check is retained as defence in depth, not because the value was actually missing. |
| F-207 sqlx migrator reconcile | done(00d6972b); prod-ledger validation folded into R-07 | `rrm-f207-sqlx-migrator`; Docker migrate-builder and both CI installation sites pin exact 0.9.0. Disposable Postgres: 0.8.6 applied all 32 migrations, 0.9.0 accepted the unchanged ledger with no pending migrations, and fresh 0.8.6/0.9.0 schema, records, and checksums matched. Exact 0.9.0 also passed the CI migration command and single-job `cargo sqlx prepare --check`. User ruling 2026-08-06: production-ledger validation is no longer a standalone pre-rollout gate - it is folded into R-07's deferred sequence (local migration test against a copy of prod data, then a prod backup, then migrations 026 and 029). |
| F-211 hanamikoji-1 delivery gap | done(85fff2e) | code half in R-16 (commit 85fff2e784e49f0191a417a1dab2325d80b5df45) |
| Pre-rollout checklist file in brdgme-config | dropped(designed out), user ruling 2026-08-06 | The three env vars (`TURNSTILE_SECRET_KEY`/`TURNSTILE_SITE_KEY`/`PUBLIC_BASE_URL`) now fail closed at startup instead of needing a checklist to catch a missing manifest value; F-207's production-ledger validation is folded into R-07; F-211 was already closed via R-16. No checklist file is written. |
...
| 4.5 4b second-reviewer rule | done(9735ead5) | `rrm-4.5-spec-code-review`: docs/CODING.md |
...
| 4.8 vendoring "known defects" spec section | done(344d94c9) | `rrm-4.8-vendoring-defects`: docs/CODING.md |
...
| D4 remediation completeness | Full ancestry audit found zero remediation items in a "claimed done, no evidence" state; remediation is complete except external and parked/backlogged items (see Work packages and Deployment items above). | 2026-08-06 |
...
- **D4 remediation completeness (user ruling 2026-08-06):** a full ancestry
  ...
  action outside this worktree - F-96 Secret sealing, R-07's production
  migration sequence) or explicitly parked/backlogged (R-31 8-player cap;
  R-37.1/R-37.2, R-43's CI job, R-46.1-4, and R-54's U8, all moved to
  `docs/BACKLOG.md`). See the Work packages and Deployment items tables above
  for the itemized disposition.
...
- **Scope / R-07 / R-22:** `R-07-HANDOVER.md` untouched; R-22 unchanged; R-21
  was gated by R-22 through R-26 and R-48 at the time (R-21/R-22 now done; see
  tracker).
...
- **R-07-HANDOVER.md:** untouched. R-07 (PARKED) status and files untouched;
  R-22/R-21 (parked/gated at the time, now done; see tracker) statuses and files
  untouched.
...
- **Scope / R-07:** `R-07-HANDOVER.md` untouched (last modified by `5c1995d`,
  pre-dating R-28; absent from the R-28 commit diff); R-07 (PARKED) status and
  files untouched; R-21/R-22 (parked/gated at the time, now done; see tracker)
  statuses and files untouched; no push.
...
- **Scope / R-07:** `R-07-HANDOVER.md` untouched (last modified by `5c1995d`,
  pre-dating R-30; absent from both R-30 commit diffs); R-07 (PARKED) status and
  files untouched; R-21/R-22 (parked/gated at the time, now done; see tracker)
  statuses and files untouched; no push.

## R-32 acceptance evidence

Status `done(0091658)`. The five implementation commits are accepted after
Lead inspection, full serial per-crate verification, and independent review:
...
- **AC4 / F-20:** the approved plan amendment classifies F-20 as not applicable.
  Starship Catan is fixed two-player, so `0..2` placings remains correct. No
  source, gameplay, player-limit, or non-two-player-fixture/test change occurred.
...
- **Verification:** static sweep found zero ungated epilogues. All 19 affected
  crates passed serial `cargo test -p`, `cargo clippy -p <crate> --all-targets --
  -D warnings`, and `cargo fmt -p <crate> -- --check`; `git diff --check` passed.
  Independent review found no blocker. Its non-blocking test-strength note is
  retained: finished-game parser rejection makes a second `command()` unavailable,
  so exercising the `was_finished == true` branch would require a new unapproved
  harness.
- **Scope:** no invalid fixture was fabricated; no Starship Catan source/gameplay
  change occurred; protected `R-07-HANDOVER.md` remains untouched.
