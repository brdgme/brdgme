# Fuzzer throughput evaluation

Evaluation of how to make the game fuzzer faster. Referenced by
`docs/BACKLOG.md` item #54 as the rationale for a future max-perf fuzzer.
Selection criterion: **throughput**. Simplicity and the non-Rust portability
argument are subordinate here (they still hold for `_repl`, where D-41
stands).

All findings are from reading the live code. **No measurements were taken -
nothing below is a measurement.** Anything not establishable by reading is in
the UNKNOWN section with the command to settle it.

Source basis (read in full): `rust/tools/fuzz/src/{main,lib}.rs`,
...
**This is a product decision, not a spec-author call**: coverage of the
...
changes). Worth it only if "fuzz everything" as one command is wanted.
...
4. **Settle the render-coverage tradeoff in (d) with an explicit decision**
   before speccing it. Default suggestion: fast path by default,
   `--check-renders` for the thorough mode, plus always doing one full
   `renders()` at game end.
...
editing. No line numbers are cited on purpose. If a function does not match what
this document describes, **STOP and report** rather than improvising.
