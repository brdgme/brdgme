...
- Game workers are Rust `warp` HTTP servers, stateless, JSON request/response, no DB/TLS/outbound calls. Listen address from `ADDR` env, default `0.0.0.0:8080`, parsed in the shared entrypoint `rust/lib/game_bin/src/lib.rs:27-31` (`brdgme_game_bin::http_main`; each game's HTTP bin is a thin wrapper). Graceful SIGTERM shutdown already implemented via `tokio::signal` in `rust/lib/cmd/src/http.rs`.
