...
**Spec:** `docs/changes/archive/2026-07-19-new-game-preview/spec.md`. Companion plan (assumed fully implemented; this plan is strictly additive on top of it and touches none of its interfaces): `docs/changes/archive/2026-07-19-new-game-page/plan.md`, which adds `rust/web/src/new_game.rs` (the `NewGamePage`/`GameBrowser` components), `GameTypeInfo`/`GameVersionInfo` with `weight`/`blurb` fields, and `db::find_game_type_player_counts`.
