...
**Spec:** docs/changes/archive/2026-07-10-28-wp4-cloudflare-pre-golive/spec.md
...
- Sealed secrets live in the sibling repo
  ...
  `docs/changes/15-production-cd-argocd/plan.md`. `kubeseal`
...
- Modify: `k8s/base/gateway/gateway.yaml` (annotation attempt; kept only on
  success)
- Modify: `docs/changes/archive/2026-07-10-28-wp4-cloudflare-pre-golive/plan.md`
...
- Modify: `docs/changes/archive/2026-07-08-20-external-dns-spec/spec.md`
...
- [ ] **Step 2: external-dns spec note.**
  `docs/changes/archive/2026-07-08-20-external-dns-spec/spec.md` already
  ...
  cross-reference "See docs/changes/archive/2026-07-08-28-abuse-protection/plan.md
  WP4." to "See docs/changes/archive/2026-07-10-28-wp4-cloudflare-pre-golive/spec.md."
- [ ] **Step 3: Commit.**
  ```sh
  cd .
  git add infra/README.md docs/changes/archive/2026-07-08-20-external-dns-spec/spec.md
...
- [x] Origin lockdown: REJECTED - accepted bypass (direct-to-LB, WP1 DB caps
  backstop). Spike 2026-07-11: `do-loadbalancer-allow-rules` set to
  Cloudflare's published ranges (22 CIDRs) on the Gateway's
  `infrastructure.annotations`. Failed - the generated Service update was
  rejected by the controller (Gateway condition Accepted=False,
  reason=NoResources, message "Unable to create Service resource"); the
  prior Service was left unchanged, so beta stayed up throughout but the
  allow-rules annotation never reached the LB. Reverted (commit 084abee),
  re-deployed, confirmed beta serves and Gateway Accepted/Programmed=True
  again.
