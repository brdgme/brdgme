...
- Do not apply migration 029. It must remain unapplied after this repair, ready for the owner to apply separately.
...
- Compile the smallest disposable Rust helper outside `rust/web` with `rustc`, not Cargo. For every input email it produces exactly `input.trim().to_lowercase()`. Its input/output stays in an operator-private file. It supplies the exact old/new values for retained rows and any approved singleton noncanonical rows.
- PostgreSQL `lower(btrim(email))` may be used only for diagnostics and migration-029 readiness checks. It is never a source of replacement data.
...
3. Inspect `_sqlx_migrations`. Require migration 029 to have no applied row. Abort if it is applied, failed, duplicated, or its state is otherwise ambiguous. Do not rerun any migration.
...
7. Use the Rust helper over every `user_emails.email`. Require the only Rust-canonical collision groups to be the two approved survivor/loser pairs; abort on any collision with an unaffected row. A noncanonical row must be an approved deleted loser row, a retained survivor row, or an approved singleton. Separately record the SQL-expression duplicate/noncanonical counts for migration-029 readiness.
...
1. Revoke loser authentication: delete loser `user_auth_tokens`; invalidate login confirmations for the bound old and canonical email values; clear loser settings and unsubscribe credentials before deletion.
...
6. Delete exactly the two approved loser `user_emails` rows. Update each retained survivor row, and each preflight-approved singleton row, with its bound Rust-helper canonical value. Assert each old value and affected row ID; no SQL expression derives the new value.
...
- Re-run the Rust helper across all stored emails: zero noncanonical rows and zero Rust-canonical duplicate groups. Re-run SQL-expression diagnostics: zero `email <> lower(btrim(email))` rows and zero `lower(btrim(email))` duplicate groups. Report counts only.
- Re-inspect `_sqlx_migrations`: 029 remains unapplied. Confirm the table now satisfies migration 029's check/index prerequisites, but do not apply 029.
