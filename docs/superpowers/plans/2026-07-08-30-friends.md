...
- [ ] **Step 1: Create `rust/web/src/friends.rs`** (full content; the
...
- [ ] **Step 2: Register the module** - in `rust/web/src/lib.rs` add after
...
- [ ] **Step 1: Extend `create_new_game`.** Change the signature (line 348)
...
- [ ] **Step 2: Enforce on restart.** In `restart_game_impl`, after the
...
- [ ] **Step 3: Fix the existing UI call site** so the crate compiles:
