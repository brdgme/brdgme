...
| 66 | F-96 Turnstile key rotation (owner-blocked) | **Owner-blocked** - captured 2026-08-07 from the 2026-07-30 review session (runbook: `docs/reviews/2026-07-30-review-session/F-96-turnstile-key.md:367-417`). Requires a human operator action that automation cannot perform: seal the new `turnstile-config` Secret (Option A) or re-seal `email-config` (Option B) via the sealed-secrets operator in `brdgme-config` - secret material handling, `read -s` discipline. Also requires bumping `brdgme-config/prod/kustomization.yaml`'s `ref=` from `0243472592e591367888cdb9a81b436dcae56688` (predates `525f81a9`) to a commit that includes the code-side `k8s/base/web/deployment.yaml` `envFrom.secretRef` wiring, once that lands. Cannot proceed without owner action. | - | - |
| 67 | Missing/broken email-address input validation (accepted a 69-char non-email string) | Captured 2026-08-07 from `docs/changes/r-07-production-email-repair/log.md`. The app accepted a 69-character sentence-like free-text string, containing a `!` and a U+00A0 non-breaking space, as the `email` value of `user_emails` row `0ad09a53-ea45-495e-ae46-820245f2bcbb` - not a valid address; the write path performed no email-shape validation. Entry point unidentified - needs investigation. Observation (not a conclusion): the garbage string's embedded address-shaped substring is verbatim R-07 Group 2's exact approved email address, which may suggest a shared root cause with the Group 2 duplicate-account situation R-07 is repairing - not investigated further. | - | - |
| 68 | Account `0b9208e6-6bd9-435b-b352-8d4dc3cff3e4` likely non-functional (unusable email) | Captured 2026-08-07 from `docs/changes/r-07-production-email-repair/log.md`. The account's only `user_emails` row is unusable free text (see #67), so the user probably cannot receive mail or log in via email. After migration 026 runs, `lower(btrim())` only lowercases the same sentence - still not a usable address. Lightly used: 1 `game_players` row, 0 `game_proposals` owned. Needs owner review; R-07 deliberately does not touch this row (see the ruling in `docs/changes/r-07-production-email-repair/log.md`). | - | - |
