...
func (g *Game) Command(
	...
	case TakeCommand:
		return g.TakeCommand(player, value.Dice, output.Remaining)
	case RollCommand:
...
func (g *Game) TakeCommand(player int, dice []int, remaining string) (brdgme.CommandResponse, error) {
	logs, err := g.Take(player, dice)
...
func (g *Game) TakeParser() brdgme.Parser {
	...
	return brdgme.Map{
		Parser: brdgme.Chain{
			brdgme.Doc{
				Name:   "take",
				Desc:   "take and score dice",
				Parser: brdgme.Token("take"),
			},
			brdgme.AfterSpace(
				brdgme.Doc{
					Name: "dice",
					Desc: "the dice to take",
					Parser: brdgme.Many{
						Min: &minNum,
						Parser: brdgme.Int{
							Min: &min,
