...
func parseBoard(input string) (board Board, err error) {
	...
	for _, l := range lines {
		...
		for j := 0; j < 10; j++ {
			board[Loc{j, i}.String()], err = parseTile(l[j*2 : (j+1)*2])
			if err != nil {
				return
			}
		}
...
// This tests the ability for the board to be properly encoded and decoded, as
// it uses complex keys.
