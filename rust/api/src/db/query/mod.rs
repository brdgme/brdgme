            ...
            for t in l.to {
                player_to.push(
                    player_id_by_position
                        .get(&t)
                        .ok_or_else::<Error, _>(|| anyhow!("no player with that position exists"))?
                        .to_owned(),
                );
            }
        ...
        for id in ids.iter() {
            users.push(
                find_user_with_primary_email(id, conn)?
                    .ok_or_else::<Error, _>(|| anyhow!("unable to find user"))?,
            );
        }
...
pub fn public_game_versions(conn: &mut PgConnection) -> Result<Vec<GameVersionType>> {
    ...
    Ok(game_versions::table
        ...
        .into_iter()
        .map(|(game_version, game_type)| GameVersionType {
            game_version,
            game_type,
        })
        .collect())
