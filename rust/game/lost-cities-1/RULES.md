...
A 2-player card game of risk and reward. Build profitable expeditions across five routes, but every expedition you start costs you 20 points - so only commit when you can make it worth it.
...
Each player draws 8 cards. The remaining 44 cards form the draw pile. Five shared discard piles (one per expedition) start empty.
...
```
score = (sum of numbered cards - 20) x (wager cards + 1)
```

- The **-20** is the expedition cost, paid whether or not you profit
- **0 wagers** = x1, **1 wager** = x2, **2 wagers** = x3, **3 wagers** = x4
- **Bonus:** +20 points if the expedition contains 8 or more cards total (including wagers)
...
The Blue expedition scores big: 2 wagers triple the value, and 9 cards (2 wagers + 7 numbered) trigger the 20-point bonus. The Yellow expedition is a costly mistake - a wager card doubled the loss from starting an expedition with too few cards.

## Rounds

The game plays 3 rounds. A round ends immediately when the draw pile is exhausted.

After scoring, the player with the higher total score starts the next round. On a tie, the other player starts.
...
- **Top section** - opponent's expeditions, most recent card at the bottom of each column; `--` means expedition started but no cards yet, colored blank means not started
- **Discard row** - the top (takeable) card on each shared pile; `--` means empty. Deck count shown on the right
- **Bottom section** - your expeditions, built the same way; most recent card at the bottom
- **Your hand** - all cards in hand, sorted by expedition then value
- **Scores** - cumulative score per round (R1, R2, R3) and running total; blank means that round hasn't been scored yet

The expedition columns are ordered **R G W B Y** left to right throughout the display.

## Strategy

- Avoid starting an expedition unless you can realistically turn a profit - the 20-point cost is punishing with few cards
