...
**3-player layout:** Both opponents are shown side by side above the discard row, then your expeditions below. (Real render to be added once a 3-player game is available.)

- **Top section** - opponent expedition columns, most recent card at the bottom of each column; `--` means expedition started but no cards yet, colored blank means not started
- **Discard row** - the top (takeable) card on each shared pile; `--` means empty. Deck count on the right
- **Bottom section** - your expeditions, built the same way
