...
- `players` (usize): Number of players in this game (2 or 3).
...
- `scores` (Vec<Vec<isize>>): Scores indexed by player, then by round. Each inner vec has one entry per completed round. Sum across rounds for the cumulative score.
- `expeditions` (Vec<Vec<Card>>): Cards played to expeditions, indexed by player. Each inner vec contains all cards that player has played across all five expeditions, in play order. Cards have an `expedition` field (Red, Green, White, Blue, Yellow) and a `value` field (Investment or N(2..10)).
